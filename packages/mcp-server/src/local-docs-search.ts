...
const EMBEDDED_METHODS: MethodEntry[] = [
  {
    name: 'list',
    endpoint: '/v1/workspaces/{workspaceId}/ai_provider_keys',
    httpMethod: 'get',
    summary: 'List AI provider keys',
    description: 'Lists all customer-provided AI provider keys in the workspace',
    stainlessPath: '(resource) ai_provider_keys > (method) list',
    qualified: 'client.aiProviderKeys.list',
    params: [
      'workspaceId: string;',
      'cursor?: string;',
      'includeInfo?: boolean;',
    ...
    response:
      "{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }; spec: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }; info?: { disabledModelCount?: number; enabledModelCount?: number; }; }",
    markdown:
      "## list\n\n`client.aiProviderKeys.list(workspaceId: string, cursor?: string, includeInfo?: boolean, limit?: number, prefix?: string, query?: string, sortOrder?: string): { metadata: resource_metadata; spec: ai_provider_key_spec; info?: object; }`\n\n**get** `/v1/workspaces/{workspaceId}/ai_provider_keys`\n\nLists all customer-provided AI provider keys in the workspace\n\n### Parameters\n\n- `workspaceId: string`\n\n- `cursor?: string`\n  Pagination cursor from previous response\n\n- `includeInfo?: boolean`\n  When true, populate each item's info (model counts), at the cost of extra\n lookups.\n\n- `limit?: number`\n  Maximum number of results to return\n\n- `prefix?: string`\n  Filter expression (query param: prefix)\n\n- `query?: string`\n  Free-form search query\n\n- `sortOrder?: string`\n  Sort order for results (asc or desc by creation time)\n\n### Returns\n\n- `{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }; spec: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }; info?: { disabledModelCount?: number; enabledModelCount?: number; }; }`\n  AIProviderKey is a customer-provided (BYOK) credential for an AI provider,\n scoped to a workspace. The secret value is never returned in responses.\n\n  - `metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }`\n  - `spec: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }`\n  - `info?: { disabledModelCount?: number; enabledModelCount?: number; }`\n\n### Example\n\n```typescript\nimport Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya();\n\n// Automatically fetches more pages as needed.\nfor await (const aiProviderKey of client.aiProviderKeys.list('workspaceId')) {\n  console.log(aiProviderKey);\n}\n```",
    perLanguage: {
      typescript: {
        method: 'client.aiProviderKeys.list',
        example:
          "import Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya({\n  apiKey: process.env['CADENYA_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const aiProviderKey of client.aiProviderKeys.list('workspaceId')) {\n  console.log(aiProviderKey.metadata);\n}",
      },
      go: {
        method: 'client.AIProviderKeys.List',
        example:
      ...
      ruby: {
        method: 'ai_provider_keys.list',
        example:
          'require "cadenya"\n\ncadenya = Cadenya::Client.new(api_key: "My API Key")\n\npage = cadenya.ai_provider_keys.list("workspaceId")\n\nputs(page)',
      },
      cli: {
        method: 'ai_provider_keys list',
        example:
          "cadenya ai-provider-keys list \\\n  --api-key 'My API Key' \\\n  --workspace-id workspaceId",
      },
      http: {
        example:
  ...
  },
  {
    ...
    endpoint: '/v1/workspaces/{workspaceId}/ai_provider_keys',
    httpMethod: 'post',
    summary: 'Create a new AI provider key',
    description: 'Creates a new customer-provided AI provider key in the workspace',
    stainlessPath: '(resource) ai_provider_keys > (method) create',
    qualified: 'client.aiProviderKeys.create',
    params: [
      'workspaceId: string;',
  ...
  {
    name: 'retrieve',
    endpoint: '/v1/workspaces/{workspaceId}/ai_provider_keys/{id}',
    httpMethod: 'get',
    summary: 'Get an AI provider key by ID',
    description: 'Retrieves an AI provider key by ID from the workspace',
    stainlessPath: '(resource) ai_provider_keys > (method) retrieve',
    qualified: 'client.aiProviderKeys.retrieve',
    params: ['workspaceId: string;', 'id: string;'],
    response:
      "{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }; spec: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }; info?: { disabledModelCount?: number; enabledModelCount?: number; }; }",
    markdown:
      "## retrieve\n\n`client.aiProviderKeys.retrieve(workspaceId: string, id: string): { metadata: resource_metadata; spec: ai_provider_key_spec; info?: object; }`\n\n**get** `/v1/workspaces/{workspaceId}/ai_provider_keys/{id}`\n\nRetrieves an AI provider key by ID from the workspace\n\n### Parameters\n\n- `workspaceId: string`\n\n- `id: string`\n\n### Returns\n\n- `{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }; spec: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }; info?: { disabledModelCount?: number; enabledModelCount?: number; }; }`\n  AIProviderKey is a customer-provided (BYOK) credential for an AI provider,\n scoped to a workspace. The secret value is never returned in responses.\n\n  - `metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }`\n  - `spec: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }`\n  - `info?: { disabledModelCount?: number; enabledModelCount?: number; }`\n\n### Example\n\n```typescript\nimport Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya();\n\nconst aiProviderKey = await client.aiProviderKeys.retrieve('id', { workspaceId: 'workspaceId' });\n\nconsole.log(aiProviderKey);\n```",
    perLanguage: {
      typescript: {
        method: 'client.aiProviderKeys.retrieve',
        example:
          "import Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya({\n  apiKey: process.env['CADENYA_API_KEY'], // This is the default and can be omitted\n});\n\nconst aiProviderKey = await client.aiProviderKeys.retrieve('id', { workspaceId: 'workspaceId' });\n\nconsole.log(aiProviderKey.metadata);",
      },
      go: {
        method: 'client.AIProviderKeys.Get',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/cadenya/cadenya-go"\n\t"github.com/cadenya/cadenya-go/option"\n)\n\nfunc main() {\n\tclient := cadenya.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\taiProviderKey, err := client.AIProviderKeys.Get(\n\t\tcontext.TODO(),\n\t\t"workspaceId",\n\t\t"id",\n\t)\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", aiProviderKey.Metadata)\n}\n',
      },
      ruby: {
        method: 'ai_provider_keys.retrieve',
        example:
          'require "cadenya"\n\ncadenya = Cadenya::Client.new(api_key: "My API Key")\n\nai_provider_key = cadenya.ai_provider_keys.retrieve("id", workspace_id: "workspaceId")\n\nputs(ai_provider_key)',
      },
      cli: {
        method: 'ai_provider_keys retrieve',
        example:
          "cadenya ai-provider-keys retrieve \\\n  --api-key 'My API Key' \\\n  --workspace-id workspaceId \\\n  --id id",
      },
      http: {
        example:
  ...
  },
  {
    ...
    description: 'Deletes an AI provider key from the workspace',
    stainlessPath: '(resource) ai_provider_keys > (method) delete',
    qualified: 'client.aiProviderKeys.delete',
    params: ['workspaceId: string;', 'id: string;'],
    markdown:
      "## delete\n\n`client.aiProviderKeys.delete(workspaceId: string, id: string): void`\n\n**delete** `/v1/workspaces/{workspaceId}/ai_provider_keys/{id}`\n\nDeletes an AI provider key from the workspace\n\n### Parameters\n\n- `workspaceId: string`\n\n- `id: string`\n\n### Example\n\n```typescript\nimport Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya();\n\nawait client.aiProviderKeys.delete('id', { workspaceId: 'workspaceId' })\n```",
    perLanguage: {
      typescript: {
        method: 'client.aiProviderKeys.delete',
        example:
          "import Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya({\n  apiKey: process.env['CADENYA_API_KEY'], // This is the default and can be omitted\n});\n\nawait client.aiProviderKeys.delete('id', { workspaceId: 'workspaceId' });",
      },
      go: {
      ...
      ruby: {
        method: 'ai_provider_keys.delete',
        example:
          'require "cadenya"\n\ncadenya = Cadenya::Client.new(api_key: "My API Key")\n\nresult = cadenya.ai_provider_keys.delete("id", workspace_id: "workspaceId")\n\nputs(result)',
      },
      cli: {
        method: 'ai_provider_keys delete',
        example:
          "cadenya ai-provider-keys delete \\\n  --api-key 'My API Key' \\\n  --workspace-id workspaceId \\\n  --id id",
      },
      http: {
        example:
          'curl https://api.cadenya.com/v1/workspaces/$WORKSPACE_ID/ai_provider_keys/$ID \\\n    -X DELETE \\\n    -H "Authorization: Bearer $CADENYA_API_KEY"',
  ...
  },
  {
    ...
    endpoint: '/v1/workspaces/{workspaceId}/ai_provider_keys/{id}',
    httpMethod: 'patch',
    summary: 'Update an AI provider key',
    description: "Updates an AI provider key's name or key value in the workspace",
    stainlessPath: '(resource) ai_provider_keys > (method) update',
    qualified: 'client.aiProviderKeys.update',
    params: [
      'workspaceId: string;',
      'id: string;',
      'metadata?: { name: string; bundleKey?: string; externalId?: string; labels?: object; };',
    ...
    response:
      "{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }; spec: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }; info?: { disabledModelCount?: number; enabledModelCount?: number; }; }",
    markdown:
      "## update\n\n`client.aiProviderKeys.update(workspaceId: string, id: string, metadata?: { name: string; bundleKey?: string; externalId?: string; labels?: object; }, spec?: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }, updateMask?: string): { metadata: resource_metadata; spec: ai_provider_key_spec; info?: object; }`\n\n**patch** `/v1/workspaces/{workspaceId}/ai_provider_keys/{id}`\n\nUpdates an AI provider key's name or key value in the workspace\n\n### Parameters\n\n- `workspaceId: string`\n\n- `id: string`\n\n- `metadata?: { name: string; bundleKey?: string; externalId?: string; labels?: object; }`\n  UpdateResourceMetadata contains the user-provided fields for updating\n a workspace-scoped resource. Read-only fields (id, account_id, workspace_id, profile_id,\n created_at) are excluded since they are set by the server.\n  - `name: string`\n    Human-readable name for the resource (e.g., \"Customer Support Agent\", \"Email Tool\")\n  - `bundleKey?: string`\n    Optional bundle ownership key. See ResourceMetadata.bundle_key.\n  - `externalId?: string`\n    External ID for the resource (e.g., a workflow ID from an external system)\n  - `labels?: object`\n    Arbitrary key-value pairs for categorization and filtering\n Examples: {\"environment\": \"production\", \"team\": \"platform\", \"version\": \"v2\"}\n\n- `spec?: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }`\n  - `apiKey?: string`\n    The provider credential. Accepted on create/update; never populated in\n responses (the server returns an empty value to avoid leaking it).\n  - `openrouter?: object`\n    OpenRouterConfig holds OpenRouter-specific settings. Empty for now; it exists\n as the oneof seam so provider-specific options (region, base URL, etc.) can be\n added later without restructuring the spec.\n  - `provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'`\n    The AI provider this key authenticates against.\n\n- `updateMask?: string`\n  Fields to update.\n\n### Returns\n\n- `{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }; spec: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }; info?: { disabledModelCount?: number; enabledModelCount?: number; }; }`\n  AIProviderKey is a customer-provided (BYOK) credential for an AI provider,\n scoped to a workspace. The secret value is never returned in responses.\n\n  - `metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }`\n  - `spec: { apiKey?: string; openrouter?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }`\n  - `info?: { disabledModelCount?: number; enabledModelCount?: number; }`\n\n### Example\n\n```typescript\nimport Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya();\n\nconst aiProviderKey = await client.aiProviderKeys.update('id', { workspaceId: 'workspaceId' });\n\nconsole.log(aiProviderKey);\n```",
    perLanguage: {
      typescript: {
        method: 'client.aiProviderKeys.update',
        example:
          "import Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya({\n  apiKey: process.env['CADENYA_API_KEY'], // This is the default and can be omitted\n});\n\nconst aiProviderKey = await client.aiProviderKeys.update('id', { workspaceId: 'workspaceId' });\n\nconsole.log(aiProviderKey.metadata);",
      },
      go: {
      ...
      ruby: {
        method: 'ai_provider_keys.update',
        example:
          'require "cadenya"\n\ncadenya = Cadenya::Client.new(api_key: "My API Key")\n\nai_provider_key = cadenya.ai_provider_keys.update("id", workspace_id: "workspaceId")\n\nputs(ai_provider_key)',
      },
      cli: {
        method: 'ai_provider_keys update',
        example:
          "cadenya ai-provider-keys update \\\n  --api-key 'My API Key' \\\n  --workspace-id workspaceId \\\n  --id id",
      },
      http: {
        example:
          "curl https://api.cadenya.com/v1/workspaces/$WORKSPACE_ID/ai_provider_keys/$ID \\\n    -X PATCH \\\n    -H 'Content-Type: application/json' \\\n    -H \"Authorization: Bearer $CADENYA_API_KEY\" \\\n    -d '{}'",
  ...
  },
  {
    ...
    response:
      "{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }; spec: { family?: string; inputPricePerMillionTokens?: string; maxInputTokens?: number; maxOutputTokens?: number; outputPricePerMillionTokens?: string; provider?: string; status?: 'MODEL_STATUS_UNSPECIFIED' | 'MODEL_STATUS_ENABLED' | 'MODEL_STATUS_DISABLED'; }; info?: { agentVariationCount?: number; aiProviderKey?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }; }",
    markdown:
      "## list\n\n`client.models.list(workspaceId: string, aiProviderKeyId?: string, bundleKey?: string, cursor?: string, includeInfo?: boolean, limit?: number, prefix?: string, query?: string, sortOrder?: string, status?: 'MODEL_STATUS_UNSPECIFIED' | 'MODEL_STATUS_ENABLED' | 'MODEL_STATUS_DISABLED'): { metadata: resource_metadata; spec: model_spec; info?: object; }`\n\n**get** `/v1/workspaces/{workspaceId}/models`\n\nLists all models in the workspace\n\n### Parameters\n\n- `workspaceId: string`\n\n- `aiProviderKeyId?: string`\n  Filter to models provisioned on a specific AI provider key. Accepts the\n key's id or an \"external_id:\"-prefixed slug.\n\n- `bundleKey?: string`\n  Filter by bundle_key — return only resources owned by this bundle.\n\n- `cursor?: string`\n  Pagination cursor from previous response\n\n- `includeInfo?: boolean`\n  When true, populate each item's info (e.g. the AI provider), at the cost of\n extra lookups.\n\n- `limit?: number`\n  Maximum number of results to return\n\n- `prefix?: string`\n  Filter by name prefix\n\n- `query?: string`\n  Free-form search query\n\n- `sortOrder?: string`\n  Sort order for results (asc or desc by creation time)\n\n- `status?: 'MODEL_STATUS_UNSPECIFIED' | 'MODEL_STATUS_ENABLED' | 'MODEL_STATUS_DISABLED'`\n  Filter by model status\n\n### Returns\n\n- `{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }; spec: { family?: string; inputPricePerMillionTokens?: string; maxInputTokens?: number; maxOutputTokens?: number; outputPricePerMillionTokens?: string; provider?: string; status?: 'MODEL_STATUS_UNSPECIFIED' | 'MODEL_STATUS_ENABLED' | 'MODEL_STATUS_DISABLED'; }; info?: { agentVariationCount?: number; aiProviderKey?: object; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }; }`\n\n  - `metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; bundleKey?: string; externalId?: string; labels?: object; }`\n  - `spec: { family?: string; inputPricePerMillionTokens?: string; maxInputTokens?: number; maxOutputTokens?: number; outputPricePerMillionTokens?: string; provider?: string; status?: 'MODEL_STATUS_UNSPECIFIED' | 'MODEL_STATUS_ENABLED' | 'MODEL_STATUS_DISABLED'; }`\n  - `info?: { agentVariationCount?: number; aiProviderKey?: { id?: string; name?: string; }; provider?: 'AI_PROVIDER_UNSPECIFIED' | 'AI_PROVIDER_OPENROUTER'; }`\n\n### Example\n\n```typescript\nimport Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya();\n\n// Automatically fetches more pages as needed.\nfor await (const model of client.models.list('workspaceId')) {\n  console.log(model);\n}\n```",
    perLanguage: {
      typescript: {
  ...
  {
    name: 'swap',
    endpoint: '/v1/workspaces/{workspaceId}/models:swapModelOnVariations',
    httpMethod: 'post',
    ...
    stainlessPath: '(resource) models > (method) swap',
    qualified: 'client.models.swap',
    params: ['workspaceId: string;', 'modelSwaps?: { currentModelId?: string; nextModelId?: string; }[];'],
    response: 'object',
    markdown:
      "## swap\n\n`client.models.swap(workspaceId: string, modelSwaps?: { currentModelId?: string; nextModelId?: string; }[]): object`\n\n**post** `/v1/workspaces/{workspaceId}/models:swapModelOnVariations`\n\nReassigns agent variations from one model to another in bulk. Runs asynchronously and returns immediately.\n\n### Parameters\n\n- `workspaceId: string`\n\n- `modelSwaps?: { currentModelId?: string; nextModelId?: string; }[]`\n  The swaps to perform.\n\n### Returns\n\n- `object`\n  Swap model on variations response. Empty: the work runs asynchronously, so no\n counts are returned (a large data set would make the call slow).\n\n### Example\n\n```typescript\nimport Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya();\n\nconst response = await client.models.swap('workspaceId');\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
        method: 'client.models.swap',
        example:
          "import Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya({\n  apiKey: process.env['CADENYA_API_KEY'], // This is the default and can be omitted\n});\n\nconst response = await client.models.swap('workspaceId');\n\nconsole.log(response);",
      },
      go: {
        method: 'client.Models.Swap',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/cadenya/cadenya-go"\n\t"github.com/cadenya/cadenya-go/option"\n)\n\nfunc main() {\n\tclient := cadenya.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tresponse, err := client.Models.Swap(\n\t\tcontext.TODO(),\n\t\t"workspaceId",\n\t\tcadenya.ModelSwapParams{},\n\t)\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", response)\n}\n',
      },
      ruby: {
        method: 'models.swap',
        example:
  ...
  },
  {
    ...
    params: ['cursor?: string;', 'limit?: number;', 'query?: string;'],
    response:
      "{ metadata: { id: string; accountId: string; name: string; profileId: string; externalId?: string; labels?: object; }; spec: { type: 'PROFILE_TYPE_UNSPECIFIED' | 'PROFILE_TYPE_USER' | 'PROFILE_TYPE_API_KEY' | 'PROFILE_TYPE_SYSTEM'; email?: string; name?: string; }; }",
    markdown:
      "## list\n\n`client.workspaceAdmin.profiles.list(cursor?: string, limit?: number, query?: string): { metadata: account_resource_metadata; spec: profile_spec; }`\n\n**get** `/v1/account/profiles`\n\nSearches the account's profiles for a member picker, with free-form name/email search and an optional type filter. Account-scoped; admin only.\n\n### Parameters\n\n- `cursor?: string`\n  Pagination cursor from previous response\n\n- `limit?: number`\n  Maximum number of results to return\n\n- `query?: string`\n  Free-form search over profile name and email. Case-insensitive substring\n match; empty returns all profiles.\n\n### Returns\n\n- `{ metadata: { id: string; accountId: string; name: string; profileId: string; externalId?: string; labels?: object; }; spec: { type: 'PROFILE_TYPE_UNSPECIFIED' | 'PROFILE_TYPE_USER' | 'PROFILE_TYPE_API_KEY' | 'PROFILE_TYPE_SYSTEM'; email?: string; name?: string; }; }`\n  A profile identifies a user or non-human principal (such as an API key)\n at the account level. Profiles are account-scoped and can be granted access\n to multiple workspaces.\n\n  - `metadata: { id: string; accountId: string; name: string; profileId: string; externalId?: string; labels?: object; }`\n  - `spec: { type: 'PROFILE_TYPE_UNSPECIFIED' | 'PROFILE_TYPE_USER' | 'PROFILE_TYPE_API_KEY' | 'PROFILE_TYPE_SYSTEM'; email?: string; name?: string; }`\n\n### Example\n\n```typescript\nimport Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya();\n\n// Automatically fetches more pages as needed.\nfor await (const profile of client.workspaceAdmin.profiles.list()) {\n  console.log(profile);\n}\n```",
    perLanguage: {
      typescript: {
