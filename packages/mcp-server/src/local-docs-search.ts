...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    response:
      "{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; externalId?: string; labels?: object; }; spec: { initialMessage: string; schedule: agent_schedule_spec_schedule; data?: object; overlapPolicy?: 'OVERLAP_POLICY_UNSPECIFIED' | 'OVERLAP_POLICY_ALLOW' | 'OVERLAP_POLICY_SKIP'; status?: string; variationId?: string; }; info?: { createdBy?: profile; lastFireAt?: string; lastObjectiveId?: string; lastSkippedAt?: string; lastSkipReason?: string; nextFireAt?: string; totalFires?: number; }; }",
    markdown:
      "## update\n\n`client.agents.schedules.update(agentId: string, id: string, metadata?: { name: string; externalId?: string; labels?: object; }, spec?: { initialMessage: string; schedule: agent_schedule_spec_schedule; data?: object; overlapPolicy?: 'OVERLAP_POLICY_UNSPECIFIED' | 'OVERLAP_POLICY_ALLOW' | 'OVERLAP_POLICY_SKIP'; status?: string; variationId?: string; }, updateMask?: string): { metadata: resource_metadata; spec: agent_schedule_spec; info?: agent_schedule_info; }`\n\n**patch** `/v1/agents/{agentId}/schedules/{id}`\n\nUpdates a schedule for an agent\n\n### Parameters\n\n- `agentId: string`\n\n- `id: string`\n\n- `metadata?: { name: string; externalId?: string; labels?: object; }`\n  UpdateResourceMetadata contains the user-provided fields for updating\n a workspace-scoped resource. Read-only fields (id, account_id, workspace_id, profile_id,\n created_at) are excluded since they are set by the server.\n  - `name: string`\n    Human-readable name for the resource (e.g., \"Customer Support Agent\", \"Email Tool\")\n  - `externalId?: string`\n    External ID for the resource (e.g., a workflow ID from an external system)\n  - `labels?: object`\n    Arbitrary key-value pairs for categorization and filtering\n Examples: {\"environment\": \"production\", \"team\": \"platform\", \"version\": \"v2\"}\n\n- `spec?: { initialMessage: string; schedule: { calendars?: schedule_calendar[]; intervals?: schedule_interval[]; timezone?: string; }; data?: object; overlapPolicy?: 'OVERLAP_POLICY_UNSPECIFIED' | 'OVERLAP_POLICY_ALLOW' | 'OVERLAP_POLICY_SKIP'; status?: string; variationId?: string; }`\n  AgentScheduleSpec is the user-provided configuration for a schedule.\n  - `initialMessage: string`\n    The initial message passed to CreateObjective on each fire. Becomes the\n first user message in the objective's chat history.\n  - `schedule: { calendars?: { comment?: string; dayOfMonth?: schedule_range[]; dayOfWeek?: schedule_range[]; hour?: schedule_range[]; minute?: schedule_range[]; month?: schedule_range[]; second?: schedule_range[]; }[]; intervals?: { every?: string; offset?: string; }[]; timezone?: string; }`\n    Schedule defines WHEN the schedule fires. Temporal-style structured form:\n a list of calendar rules (wall-clock) and/or interval rules (duration),\n OR'd together. At least one rule is required.\n  - `data?: object`\n    Optional input data passed to the objective. If the agent has an\n input_data_schema, this must satisfy it.\n  - `overlapPolicy?: 'OVERLAP_POLICY_UNSPECIFIED' | 'OVERLAP_POLICY_ALLOW' | 'OVERLAP_POLICY_SKIP'`\n    What to do when the previous run is still in flight. Defaults to SKIP.\n  - `status?: string`\n    Lifecycle. Defaults to ACTIVE on create when unspecified.\n  - `variationId?: string`\n    Optional explicit variation. When unset, the agent's variation_selection_mode\n chooses per fire.\n\n- `updateMask?: string`\n  Fields to update.\n\n### Returns\n\n- `{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; externalId?: string; labels?: object; }; spec: { initialMessage: string; schedule: agent_schedule_spec_schedule; data?: object; overlapPolicy?: 'OVERLAP_POLICY_UNSPECIFIED' | 'OVERLAP_POLICY_ALLOW' | 'OVERLAP_POLICY_SKIP'; status?: string; variationId?: string; }; info?: { createdBy?: profile; lastFireAt?: string; lastObjectiveId?: string; lastSkippedAt?: string; lastSkipReason?: string; nextFireAt?: string; totalFires?: number; }; }`\n  AgentSchedule resource — a recurring trigger attached to an agent that\n creates objectives on its cadence.\n\n  - `metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; externalId?: string; labels?: object; }`\n  - `spec: { initialMessage: string; schedule: { calendars?: schedule_calendar[]; intervals?: schedule_interval[]; timezone?: string; }; data?: object; overlapPolicy?: 'OVERLAP_POLICY_UNSPECIFIED' | 'OVERLAP_POLICY_ALLOW' | 'OVERLAP_POLICY_SKIP'; status?: string; variationId?: string; }`\n  - `info?: { createdBy?: { metadata: account_resource_metadata; spec: profile_spec; }; lastFireAt?: string; lastObjectiveId?: string; lastSkippedAt?: string; lastSkipReason?: string; nextFireAt?: string; totalFires?: number; }`\n\n### Example\n\n```typescript\nimport Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya();\n\nconst agentSchedule = await client.agents.schedules.update('id', { agentId: 'agentId' });\n\nconsole.log(agentSchedule);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      "{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; externalId?: string; labels?: object; }; spec: { config: tool_spec_config; description: string; parameters: object; status: 'TOOL_STATUS_UNSPECIFIED' | 'TOOL_STATUS_AVAILABLE' | 'TOOL_STATUS_OMITTED' | 'TOOL_STATUS_ARCHIVED'; requiresApproval?: boolean; }; info?: { createdBy?: profile; toolSet?: resource_metadata; }; }",
    markdown:
      "## update\n\n`client.toolSets.tools.update(toolSetId: string, id: string, metadata?: { name: string; externalId?: string; labels?: object; }, spec?: { config: tool_spec_config; description: string; parameters: object; status: 'TOOL_STATUS_UNSPECIFIED' | 'TOOL_STATUS_AVAILABLE' | 'TOOL_STATUS_OMITTED' | 'TOOL_STATUS_ARCHIVED'; requiresApproval?: boolean; }, updateMask?: string): { metadata: resource_metadata; spec: tool_spec; info?: tool_info; }`\n\n**put** `/v1/tool_sets/{toolSetId}/tools/{id}`\n\nUpdates a tool in the tool set\n\n### Parameters\n\n- `toolSetId: string`\n\n- `id: string`\n\n- `metadata?: { name: string; externalId?: string; labels?: object; }`\n  UpdateResourceMetadata contains the user-provided fields for updating\n a workspace-scoped resource. Read-only fields (id, account_id, workspace_id, profile_id,\n created_at) are excluded since they are set by the server.\n  - `name: string`\n    Human-readable name for the resource (e.g., \"Customer Support Agent\", \"Email Tool\")\n  - `externalId?: string`\n    External ID for the resource (e.g., a workflow ID from an external system)\n  - `labels?: object`\n    Arbitrary key-value pairs for categorization and filtering\n Examples: {\"environment\": \"production\", \"team\": \"platform\", \"version\": \"v2\"}\n\n- `spec?: { config: { http?: config_http; mcp?: config_mcp; }; description: string; parameters: object; status: 'TOOL_STATUS_UNSPECIFIED' | 'TOOL_STATUS_AVAILABLE' | 'TOOL_STATUS_OMITTED' | 'TOOL_STATUS_ARCHIVED'; requiresApproval?: boolean; }`\n  - `config: { http?: { requestMethod: 'HTTP_METHOD_UNSPECIFIED' | 'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE'; headers?: object; path?: string; query?: string; requestBodyContentType?: string; requestBodyTemplate?: string; toolName?: string; }; mcp?: { toolDescription?: string; toolName?: string; toolTitle?: string; }; }`\n    Config defines the adapter to use for the tool.\n This is used to determine how the tool is called.\n For example, if the tool is an HTTP tool, the adapter will be Http.\n If the tool is an inline tool, the adapter will be Inline.\n  - `description: string`\n  - `parameters: object`\n  - `status: 'TOOL_STATUS_UNSPECIFIED' | 'TOOL_STATUS_AVAILABLE' | 'TOOL_STATUS_OMITTED' | 'TOOL_STATUS_ARCHIVED'`\n  - `requiresApproval?: boolean`\n\n- `updateMask?: string`\n\n### Returns\n\n- `{ metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; externalId?: string; labels?: object; }; spec: { config: tool_spec_config; description: string; parameters: object; status: 'TOOL_STATUS_UNSPECIFIED' | 'TOOL_STATUS_AVAILABLE' | 'TOOL_STATUS_OMITTED' | 'TOOL_STATUS_ARCHIVED'; requiresApproval?: boolean; }; info?: { createdBy?: profile; toolSet?: resource_metadata; }; }`\n\n  - `metadata: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; externalId?: string; labels?: object; }`\n  - `spec: { config: { http?: config_http; mcp?: config_mcp; }; description: string; parameters: object; status: 'TOOL_STATUS_UNSPECIFIED' | 'TOOL_STATUS_AVAILABLE' | 'TOOL_STATUS_OMITTED' | 'TOOL_STATUS_ARCHIVED'; requiresApproval?: boolean; }`\n  - `info?: { createdBy?: { metadata: account_resource_metadata; spec: profile_spec; }; toolSet?: { id: string; accountId: string; createdAt: string; name: string; profileId: string; workspaceId: string; externalId?: string; labels?: object; }; }`\n\n### Example\n\n```typescript\nimport Cadenya from '@cadenya/cadenya';\n\nconst client = new Cadenya();\n\nconst tool = await client.toolSets.tools.update('id', { toolSetId: 'toolSetId' });\n\nconsole.log(tool);\n```",
    perLanguage: {
      typescript: {
