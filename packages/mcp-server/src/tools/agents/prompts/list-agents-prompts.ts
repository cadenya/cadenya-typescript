...
export const handler = async (client: Cadenya, args: Record<string, unknown> | undefined) => {
  ...
  const response = await client.agents.prompts.list(agentId, body).asResponse();
