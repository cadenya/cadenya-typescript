# Cadenya TypeScript SDK

The official TypeScript client for the Cadenya API. Generated by redwood.
...
const result = await client.accounts.retrieve();
```
...
```ts
const client = new Cadenya({ logLevel: 'debug' }); // or logger: myLogger
```
...
const stream = await client.objectives.streamEvents(objectiveId, params);
...
const envelopes = await client.objectives.streamEvents(objectiveId, params);
...
```ts
const resumed = await client.objectives.streamEvents(objectiveId, params, {
...
```ts
import { Webhooks } from '@cadenya/cadenya';

const webhooks = new Webhooks(process.env.CADENYA_WEBHOOK_SECRET);
